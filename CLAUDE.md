# markcampbell.ai

> **Location:** `~/Documents/Code/markcampbell-ai/`
> **Live:** https://markcampbell.ai

## Project Links
- **Drive:** My Drive/Claude Code Projects/markcampbell.ai/
- **GitHub:** https://github.com/macsoup-max/markcampbell-ai
- **Domain:** Namecheap (macsoup@gmail.com), expires 2028-03-18

## Working Folders
| Folder | Permission |
|--------|------------|
| ~/Documents/Code/markcampbell-ai/ | Read + Write |
| My Drive/Claude Code Projects/markcampbell.ai/ | Read + Write |

## Brand
- **Color:** Logic Blue #007AFF
- **Font:** Plus Jakarta Sans
- **Design:** Institutional/executive, zero creator-brand patterns

## Deploy
Push to main branch. GitHub Pages auto-deploys in 1-2 minutes.
